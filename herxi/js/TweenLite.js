...
var TweenQuick = (function (){
	var cache 		= 	[],
		$spinner    =   $('<span class="spinner icon"/>'),
	...
	slideHorizontally		=	function( $element, reverse, callback ) {
		...
		if(timeline!=null){
			...
			if( height == 0 ){
    			height          =   $element.height('auto').height();
    			$element.height(0);
			}
			...
			step1.speed				=	0.05+step1.distance/250;
			...
			step2.speed				=	0.1+step2.distance/1250;
	...
	replaceContent		=	function( $element, replacement, extraCss, callback ) {
        ...
        currentHtml         =   currentHtml.filter(function(){
    ...
    turnButtonIntoProgressButton    =   function( $button ){
        var $buttonIcons    =    $button.find('.icon');
        var timeline        =   new TimelineLite({align:'sequel', onReverseComplete:function(){
            $spinner.detach();
            var defaultText     =   $button.attr('data-original');
            if( defaultText ){
                setTimeout( function(){
        ...
        timeline.to( $button, 0.5, { color : "#adadb5", background : '#adadb5', borderColor : '#999'} );
        timeline.insert( TweenLite.fromTo( $buttonIcons, 0.5, { autoAlpha : 1 }, { autoAlpha : 0 } ) );
        timeline.insert( TweenLite.fromTo( $spinner, 0.3, { autoAlpha : 0 }, { autoAlpha : 1, delay : 0.2 } ) );
        timeline.insert( TweenLite.fromTo( $spinner, 0.5, { scale : 2 }, { scale : 1, delay : 0.2 } ) );
    ...
    return {
        ...
        turnButtonIntoProgressButton:   turnButtonIntoProgressButton
    };
}());