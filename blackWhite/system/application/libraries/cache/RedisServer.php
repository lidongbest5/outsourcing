<?php
require_once realpath ( dirname ( __FILE__ ) ) . '/IRedisServer.php';
/**
...
class RedisServer implements IRedisServer {
	...
	public function connect($host, $port) {
		...
		if (! $socket)
			return $this->ReportError ( 'Connection error: ' . $errno . ':' . $errstr, __LINE__ );
		return $socket;
	}
				...
				if ($size > 0) {
					do {
						...
						if ($block_size < 1)
							break;
						if ($chi > 1000)
							return $this->ReportError ( 'loooop', __LINE__ );
	...
	public function PUnsubscribe($patterns = null) {
		if (! empty ( $patterns )) {
			...
			return $this->send_command ( 'PUnsubscribe' );
	}
	...
	public function Unsubscribe($channel = '') {
		$args = func_get_args ();
		if (empty ( $args ))
			return $this->send_command ( 'Unsubscribe' );
		else {
			if (is_array ( $channel ))
	...
	public function zRange($key, $start, $stop, $withscores = false) {
		if ($withscores)
			return $this->send_command ( 'zRange', $key, $start, $stop, self::WITHSCORES );
		else
			return $this->send_command ( 'zRange', $key, $start, $stop );
	}
	...
	public function zRevRange($key, $start, $stop, $withscores = false) {
		if ($withscores)
			return $this->send_command ( 'zRevRange', $key, $start, $stop, self::WITHSCORES );
		else
			return $this->send_command ( 'zRevRange', $key, $start, $stop );
	}
