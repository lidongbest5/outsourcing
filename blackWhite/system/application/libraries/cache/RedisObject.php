...
require_once realpath ( dirname ( __FILE__ ) ) . '/RedisServer.php';
...
class RedisObject extends MemoryObject implements IMemoryStorage {
	...
	public function rPush($k, $v) {
		$this->redis->RPush ( $this->prefix . $k, $v );
		return true;
	}
	
	public function LPop($k) {
		$r = $this->redis->LPop ( $this->prefix . $k );
		if ($r === false)
			return false;
		return $r;
	}
	...
	public function LPush($k, $v) {
		$this->redis->LPush ( $this->prefix . $k, $v );
		return true;
	}
	
	public function RPop($k) {
		$r = $this->redis->RPop ( $this->prefix . $k );
		if ($r === false)
			return false;
		return $r;
	}
	...
	public function increment($key, $by_value = 1, $limit_keys_count = 0) {
		...
		if (! $this->acquire_key ( $key, $auto_unlocker ))
			return $this->ReportError ( 'Can not acquire key', __LINE__ );
