...
class MBOpenTOAuth {
	...
	public $format = 'json';
	public $mb_api_host = 'open.t.qq.com';
    ...
    function __construct($consumer_key, $consumer_secret, $api_host, $format = 'json', $oauth_token = NULL, $oauth_token_secret = NULL) {

		$this->mb_api_host = $api_host;
		$this->format = $format;
	...
	* Exchange the request token and secret for an access token and
	...
	function http($url, $method, $postfields = NULL , $multi = false){
		...
		if(strrpos($url, 'https://')===0){
			$port = 443;
			$version = '1.1';
			$host = 'ssl://'.$this->mb_api_host;

		}else{
			$port = 80;
			$version = '1.0';
			$host = $this->mb_api_host;
		}

		$header = "$method $url HTTP/$version\r\n";
		$header .= "Host: ".$this->mb_api_host."\r\n";
		if($multi){
