...
require_once realpath ( dirname ( __FILE__ ) . '/../' ) . '/MemoryObject.php';
...
class SHMObject extends MemoryObject implements IMemoryStorage {
	...
	public function save($k, $v, $ttl = 259200, $tags = NULL) {
		...
		if (empty ( $map ))
			...
			if (! array_key_exists ( $k, $map )) {
				...
				if ($size <= ($map [$k] [self::map_key_fin] - $map [$k] [self::map_key_start]))
					$start = $map [$k] [self::map_key_start];
				else {
					$this->del ( $k );
					$this->del_old ();
					$map = $this->mem_object->read ( 'map' );
		...
		if ($set_ttl > 0)
			$map [$k] [self::map_key_ttl] = $set_ttl;
		if ($data_serialized)
			$map [$k] [self::map_key_serialized] = $data_serialized;
	...
	public function sort_map($a, $b) {
		if ($a [self::map_key_start] == $b [self::map_key_start])
			return 0;
		if ($a [self::map_key_start] < $b [self::map_key_start])
			return - 1;
		else
	...
	public function read($k, &$ttl_left = -1) {
		...
		if (is_array ( $k )) {
			...
			if ($map [$k] [self::map_key_serialized] == 1)
