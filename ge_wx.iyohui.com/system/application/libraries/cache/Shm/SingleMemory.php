...
require_once realpath ( dirname ( __FILE__ ) ) . '/ISingleMemory.php';
...
abstract class SingleMemory extends MemoryObject implements ISingleMemory {
	...
	public function save($k, $v, $ttl = 2592000, $tags = NULL) {
		...
		$ttl = intval ( $ttl );
		if ($ttl > 0)
			$this->mem [self::map_key_ttl] [$k] = time () + $ttl;
		if (! empty ( $tags )) {
			if (! is_array ( $tags ))
				$tags = array ($tags );
			foreach ( $tags as $tag ) {
				if (empty ( $this->mem [self::map_key_tags] [$tag] ) || ! in_array ( $k, $this->mem [self::map_key_tags] [$tag] ))
	...
	public function get_keys() {
		$this->readmemory ();
		if (! empty ( $this->mem [self::map_keys] ))
			return array_keys ( $this->mem [self::map_keys] );
		else
			return array ();
	}
